...
for name in ['sp']:
       ...
       # read training data for GAN 13y的数据用来增强
       if name == 'hst':
              dataset = pd.read_csv('data/sp.csv')
       ...
       # define time steps for prediction, n_tr_months = 49
       n_months = 12
       # n_te_months是测试集，n_tr_months是训练集 测试集长度，论文当中有提到
       n_te_months = 24
       ...
       # 最终从生成的窗口数据中提取出前n_tr_months个窗口数据作为GAN模型的训练集。
       train_gan_x = features_gan[:n_tr_months, :, :]

       # training rnn gan
       params1  = {"epoch": 20000, "module_name": 'gru', "batch_size": 5,
                   "hidden_size": 8, "num_layer": 2, "z_dim": 3, "lr": 0.0001 }
